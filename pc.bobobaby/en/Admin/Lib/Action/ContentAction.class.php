...
class ContentAction extends CommonAction {
	...
	public function addContent(){
		...
		if(($catid==23 || $catid==24) && $siteid==2){
			$pro_list=$this->pro_db->field('a.id,en_product.title,a.capacity,a.price')->join("left join `en_product_price` a on en_product.id=a.pid")->where('en_product.status=1')->select();
