...
<!--内容-->
<div id="content">
<div id="breadcrumb"> <a href="__APP__/Index/index" title="Go to Home" class="tip-bottom"><i class="icon-home"></i>主页</a>->编辑联动菜单</span></div>
<div class="quick-actions_homepage">
 ...
 <div class="row-fluid">
     ...
     <div class="span12">
		...
		<form action="__APP__/Link/doLink/action/edit/linkageid/<?php echo ($link_arr["linkageid"]); ?>" id="myform" class="myForm" method="post" >	
     	<table class="table table-bordered table-striped">


     	  <tr >		  <td width="100px"><span>一级菜单名称：</span></td>		  <td>			<select name="info[catid]" datatype="*" nullmsg="请选择一级菜单">				<option value="">请选择一级菜单</option>				<?php if(is_array($cate_list)): foreach($cate_list as $key=>$v): ?><option <?php if($v["catid"] == $link_arr[catid]): ?>selected<?php endif; ?> value="<?php echo ($v["catid"]); ?>"><?php echo ($v["catname"]); ?></option><?php endforeach; endif; ?>				</select>		  </td>		  </tr>					     			  <tr >		  <td width="100px"><span>二级菜单名称：</span></td>		  <td>			 <input type="text" name="info[name]" datatype="*" nullmsg="菜单名不能为空" value="<?php echo ($link_arr["name"]); ?>"/>		  </td>		  </tr>
  ...
  $('.myForm').Validform({
				...
				callback:function(data){
					...
					if(data.status=='y'){
						...
						setTimeout(function(){window.location="__APP__/Link/index/cls/3";},1000);
