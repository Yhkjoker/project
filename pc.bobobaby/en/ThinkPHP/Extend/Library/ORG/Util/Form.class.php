...
class form {
    ...
    function form_checkbox($name,$label=array(),$label_name,$label_for=""){
        ...
        foreach ($label as $id=>$value){
            $text.="<input type=\"checkbox\" id=\"{$id}\" name=\"{$name}[]\" value=\"{$value}\"  datatype=\"*\" nullmsg=\"请选择信息\"/>";
            $text.="<span for=\"{$id}\">{$value}</span><br/>";
    ...
    function form_radio($name,$label=array(),$label_name,$label_for=""){
        ...
        $text="";
        foreach ($label as $id=>$value){
            $text.="<input type=\"radio\" id=\"{$id}\" name=\"{$name}\" value=\"{$value}\" datatype=\"*\" nullmsg=\"请选择信息\" />";
            $text.="<span for=\"{$id}\">{$value}</span><br/>";
    ...
    function CreateForm($form_item=array()){
		$string="";
        // $string.= $this->form_start();
        foreach ($form_item as $item){
            $string.= $item;
        }
        // $string.= $this->form_end();
		return $string;
    }
}
?>