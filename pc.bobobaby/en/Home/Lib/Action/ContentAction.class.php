...
class ContentAction extends CommonAction {
	...
	public function news_xq(){
		...
		if($catid==23 && !empty($con_arr['productlist'])){
						$pro_list=$this->pro_db->field("en_product.picurlpc,en_product.title,en_product.catid,a.id,a.pid,a.capacity,a.price,b.catname,a.colorlist")->join("left join en_product_price a on en_product.id=a.pid left join en_category b on en_product.catid=b.catid")->where("en_product.status=1 and a.id in (".$con_arr['productlist'].")")->limit(6)->select();
		...
		if($catid==24){
			$pro_list=$this->pro_db->field("en_product.picurlpc,en_product.title,en_product.catid,a.id,a.pid,a.capacity,a.price,b.catname")->join("left join en_product_price a on en_product.id=a.pid left join en_category b on en_product.catid=b.catid")->where("en_product.status=1 and a.id in (".$con_arr['productlist'].")")->limit(6)->select();
			$this->assign("pro_list",$pro_list);
