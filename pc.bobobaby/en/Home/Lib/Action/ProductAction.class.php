...
class ProductAction extends CommonAction {
    ...
    public function product(){
		...
		$pro_list_fw=$this->pro_db->field("en_product.xqstatus,en_product.status,en_product.picurlpc,en_product.title,en_product.colorlist,a.price,a.capacity,en_product.id as pid,en_product.catid,a.id")->join("left join en_product_price a on en_product.id=a.pid")->where($where)->limit(5)->order('en_product.listorder desc,en_product.id desc')->select();
		...
		if($catid){	
			...
			if($link_count>0){
				...
				foreach($link_list as $k=>$v){
					$pro_list=$this->pro_db->field("a.xqstatus,a.title,a.picurlpc,a.catid,a.id,b.capacity,b.price,b.id as ppid,a.colorlist,b.fmpicurl")->table("en_product_price b,en_product a")->where("a.catid=$catid and a.linkageid=".$v['linkageid']." and a.status=1 and  a.id=b.pid".$where)->order("listorder desc")->select();
				...
				$where.="a.catid=$catid  and a.status=1 and  a.id=b.pid";
					...
					$pro_list=$this->pro_db->field("a.xqstatus,a.title,a.picurlpc,a.catid,a.id,b.capacity,b.price,b.id as ppid,a.colorlist,b.fmpicurl")->table("en_product_price b,en_product a")->where($where)->order("a.listorder desc,id desc")->select();
	...
	public function product_all(){
		...
		$pro_list_fw=$this->pro_db->field("en_product.xqstatus,en_product.status,en_product.picurlpc,en_product.title,en_product.colorlist,a.price,a.capacity,en_product.id as pid,en_product.catid,a.id")->join("left join en_product_price a on en_product.id=a.pid")->where($where_zj)->limit(5)->order('en_product.listorder desc,en_product.id desc')->select();
			foreach ($pro_list_fw as $k=>$v){
					$where_two['id']=array('in',$v['colorlist']);
					$where_two['status']=1;
					$para_list_one=$this->para_db->field("colorname,name")->where($where_two)->select();
	...
	public function product_details(){
		...
		$pro_list=$this->pro_db->field("en_product.picurlpc,en_product.title,en_product.colorlist,a.price,a.capacity,en_product.id as pid,en_product.catid,a.id")->join("left join en_product_price a on en_product.id=a.pid")->where("en_product.catid=$catid  and en_product.status=1 ")->limit(6)->order('en_product.listorder desc,en_product.id desc')->select();
		...
		$cate_arr=$this->cate_db->field("catname")->where("catid=$catid")->find();
		$pro_arr=$this->pro_db->field("en_product.xqstatus,en_product.content,en_product.picurlpcall,en_product.title,en_product.colorlist,a.price,c.id as colid,a.id as ppid")->join("left join en_product_price a on en_product.id=a.pid left join en_collect c on a.id=c.ppid")->where("en_product.catid=$catid and en_product.id=$pid and en_product.status=1 and a.id=$ppid")->find();
