...
class MemberAction extends CommonAction {
	...
	public function product(){
		...
		if(empty($sess_id)){
			...
			$pro_list=$this->collect_db->field("a.xqstatus,a.title,a.picurlpc,a.catid,a.id,b.capacity,b.price,b.id as ppid,a.colorlist,b.fmpicurl")->table("en_collect c,en_product_price b,en_product a")->where($where)->order("a.listorder desc,id desc")->select();
	...
	public function addTryPro(){
		...
		if(!empty($sess_id)){
			...
			$data['vid']=$sess_id;
			$data['inputtime']=time();
			$pro_price_count=$this->pro_price_db->join("left join en_product a on en_product_price.pid=a.id")->where(array('a.actstatus'=>1,'a.status'=>1,'en_product_price.id'=>$data['ppid']))->count();
