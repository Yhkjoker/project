...
<?php } ?>
        ...
        <div class="Product_list_content shiyong">
            ...
            <div class="Product_list_right f_right">
                ...
                <div class="Product_list_right_SYcp">
                    <ul>
					<?php if($pro_list ): if(is_array($pro_list)): foreach($pro_list as $key=>$v): ?><li>
								<a href="<?php echo U('Try/product_details',array('catid'=>$v[catid],'pid'=>$v[pid],'ppid'=>$v[ppid]));?>"><img class="Product_list_right_LiImg" src="<?php echo ($v["picurlpc"]); ?>" />
								<div class="Product_list_right_SYcp_DW"></div> 
								<h2><?php echo ($v["title"]); ?><span><?php echo ($v["capacity"]); ?></span></h2></a>
								 <?php  session_start(); if(!$_SESSION['bobo_pc_user_id']){?>
								<input  class="Product_list_right_SYcp_Btn1" type="button" value="申请试用" onclick="hhhhh();"/>
								<?php }else{ ?>
								<input class="Product_list_right_SYcp_Btn1" type="button" value="申请试用" onclick="wysy(<?php echo ($v["ppid"]); ?>);"/>
        ...
        <!-- 2016-05-24修改弹窗样式 -->
         <style>
         .cuoico { background:url("/bobo2/Public/img/iconc1.png") no-repeat !important;}
         </style>
