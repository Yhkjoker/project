        ...
        <div class="Product_list_content shiyong">
            ...
            <div class="Product_list_right f_right">
                ...
                <div class="Product_list_right_SYcp">
                    <ul>
					<?php if($try_list ): if(is_array($try_list)): foreach($try_list as $key=>$v): if($v["status"] == 1 || $v["status"] == 2 || $v["status"] == 3 || $v["status"] == 4): ?><li>
								<img class="Product_list_right_LiImg" src="<?php echo ($v["picurlpc"]); ?>" />
								<div class="Product_list_right_SYcp_DW"></div> 
								<h2><?php echo ($v["title"]); ?><span><?php echo ($v["capacity"]); ?></span></h2>
								<input class="Product_list_right_SYcp_Btn1" type="button" value="查看进度" onclick="window.location.href='<?php echo U('Try/schedule',array('cls'=>$_GET[cls],'id'=>$v[id]));?>'"/>
							</li>
						<?php elseif($v["status"] == 5): ?>
							<li>
								...
								<div class="Product_list_right_SYcp_DW"></div> 
								<h2><?php echo ($v["title"]); ?><span><?php echo ($v["capacity"]); ?></span></h2>
								<input class="Product_list_right_SYcp_Btn2" type="button" value="试用结束" onclick="window.location.href='<?php echo U('Try/schedule',array('cls'=>$_GET[cls],'id'=>$v[id]));?>'"/>
