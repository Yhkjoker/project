...
class TryAction extends CommonAction{
	...
	public function index(){
		...
		$count=$this->try_out_db->join("left join `tp_vipuser` a on tp_try_out.vid=a.vid left join `tp_product_price` b on tp_try_out.ppid=b.id left join `tp_product` c on b.pid=c.id")->where(array('tp_try_out.status'=>$status))->count();
		$page  = new Page($count,10);
		$show=$page->show();
		$try_list=$this->try_out_db->field("tp_try_out.id,a.username,c.title,b.capacity,tp_try_out.inputtime,tp_try_out.status")->join("left join `tp_vipuser` a on tp_try_out.vid=a.vid left join `tp_product_price` b on tp_try_out.ppid=b.id left join `tp_product` c on b.pid=c.id")->where(array('tp_try_out.status'=>$status))->limit($page->firstRow.','.$page->listRows)->order("tp_try_out.id desc")->select();
	...
	public function conSearch(){
		...
		$where="";
		if($factor==1){
			$where.="a.username ='$search' and tp_try_out.status=$status";
		}elseif($factor==2){
			
			$where.="c.title like '$search' and tp_try_out.status=$status";
		}
		
		import('ORG.Util.Page');
		$count=$this->try_out_db->join("left join `tp_vipuser` a on tp_try_out.vid=a.vid left join `tp_product_price` b on tp_try_out.ppid=b.id left join `tp_product` c on b.pid=c.id")->where($where)->count();
		$page  = new Page($count,10);
		$show=$page->show();
		$try_list=$this->try_out_db->field("tp_try_out.id,a.username,c.title,b.capacity,tp_try_out.inputtime,tp_try_out.status")->join("left join `tp_vipuser` a on tp_try_out.vid=a.vid left join `tp_product_price` b on tp_try_out.ppid=b.id left join `tp_product` c on b.pid=c.id")->where($where)->limit($page->firstRow.','.$page->listRows)->order("tp_try_out.id desc")->select();
		$this->assign("show",$show);
		$this->assign("try_list",$try_list);
		$this->display();
	}
	...
	public function selMessage(){
		$id=intval($_GET['id']);
		$try_arr=$this->try_out_db->field("tp_try_out.name,tp_try_out.phone,tp_try_out.address,a.username,c.title,b.capacity,tp_try_out.inputtime,tp_try_out.status,tp_try_out.evaluate")->join("left join `tp_vipuser` a on tp_try_out.vid=a.vid left join `tp_product_price` b on tp_try_out.ppid=b.id left join `tp_product` c on b.pid=c.id")->where(array('tp_try_out.id'=>$id))->find();
