 ...
 <div class="row-fluid">
     ...
     <div class="span12">
        ...
        <div class="widget-box">
          ...
          <form  action="__APP__/Product/doProduct/action/edit/id/<?php echo ($pro_arr["id"]); ?>" method="post" enctype="multipart/form-data">
           ...
           <table class="table table-bordered table-striped">
			...
			<tr>
			  ...
			  <td><input type="text" name="info[title]" datatype="*" nullmsg="标题为空" value="<?php echo ($pro_arr["title"]); ?>"/>
			...
			<tr>
			  ...
			  <td>
				<select name="info[linkageid]">
					...
					<?php if(is_array($link_list)): foreach($link_list as $key=>$v): ?><option <?php if($pro_arr["linkageid"] == $v[linkageid]): ?>selected<?php endif; ?> value="<?php echo ($v["linkageid"]); ?>"><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
				</select>
			  </td>
			...
			<tr>
			  ...
			  <td>
				<select name="info[pmid]">
					...
					<?php if(is_array($problem_list)): foreach($problem_list as $key=>$v): ?><option <?php if($pro_arr["pmid"] == $v[id]): ?>selected<?php endif; ?> value="<?php echo ($v["id"]); ?>"><?php echo ($v["title"]); ?></option><?php endforeach; endif; ?>
				</select>
			  </td>
             	...
             	<td><textarea name="info[activity]" id="ke_demo_one" style="width:800px;height:350px;" datatype="*" ignore="ignore" nullmsg="不能为空"><?php echo ($pro_arr["activity"]); ?></textarea></td>
             ...
             <tr>
             	...
             	<td><textarea name="info[content]" id="ke_demo" style="width:800px;height:350px;" datatype="*" ignore="ignore" nullmsg="不能为空"><?php echo ($pro_arr["content"]); ?></textarea></td>

             </tr>
			<tr><td><span>上传pc站封面图片：</span></td><td>
			<input type="text" id="url3" name="info[picurlpc]" value="<?php echo ($pro_arr["picurlpc"]); ?>" class="ke-input-text"/> <input type="button" id="image3" class="btn btn-success delpic" value="选择图片" style="margin-bottom:10px"/>
</td></tr>
					...
					<tr><td><span> 价格信息：</span></td><td>
						<table width="950" >
							...
							<tbody>
							 ...
							 <tbody id="addTrr">
						<?php if(is_array($price_list)): foreach($price_list as $k=>$v): ?><tr id="d_<?php echo ($v["id"]); ?>">
									<td>
											<input type="text"  style="width:100px;" name="info_price_up[<?php echo ($v["id"]); ?>][capacity]" value="<?php echo ($v["capacity"]); ?>"/>	
									</td>
									<td>
											<input type="text"  style="width:100px;"  name="info_price_up[<?php echo ($v["id"]); ?>][price]" value="<?php echo ($v["price"]); ?>"/>
									</td>
									  <td >
									<ul >
								  <?php if(is_array($texture_list)): foreach($texture_list as $key=>$vo): ?><li style="float:left;"><div><input type="checkbox" name="info_price_up[<?php echo ($v["id"]); ?>][texturelist][]" value="<?php echo ($vo["id"]); ?>"  <?php if(is_array($v[texturelist])): foreach($v[texturelist] as $key=>$val): if($val == $vo[id]): ?>checked<?php endif; endforeach; endif; ?>><?php echo ($vo["colorname"]); ?></div></li><?php endforeach; endif; ?>
									</ul>
								  </td>
									
									<td>
												<input type="file"  name="fmpicurl[]"  class="ke-input-text" /><img src="<?php echo ($v["fmpicurl"]); ?>" width="70" height="39"/>
									</td>
		...
		<tr><td colspan="2">
				<div id="J_imageView">
				<?php
 if($pro_arr['picurlpcall']){ $pic_list=explode(",",$pro_arr['picurlpcall']); $color_list=explode(",",$pro_arr['colorlist']); $data=""; foreach($pic_list as $ke=>$va){ $data.="<div id='img$ke'>"; foreach($para_list as $k=>$vo){ $data.= '<div class="htdian" style="width:100px;"><span><input type="radio" name="info[colorlist]['.$ke.']" value="'.$vo['id'].'" '; if($color_list[$ke]==$vo['id'] || $k==0){ $data.='checked'; } $data.=' item="val"></span><span class="httxt">'.$vo['name'].'</span><span class="htdiansp" style="width: 20px;height: 20px;display: block;background-color:'.$vo['colorname'].';"></span></div>'; } $data.="<p><img src='$va' class='allImg htphoto' width='200px' heigth='200px'/></p><a href='javascript:shanchu($ke)' >delete</a><hr/></div>"; } echo $data; } ?>
		</div>
			...
			<tr>
			  ...
			  <td><input type="hidden" name="info[picurlpcall]" id="purl" value="<?php echo ($pro_arr["picurlpcall"]); ?>"/><input type='text' name='info[updatetime]' value="<?php  echo date('Y-m-d H:i:s',time()); ?>" class='text-input small-input Wdate'  onBlur="FireEvent(this,'onchange');" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})"   readonly=""/></td>
...
$(function(){
    ...
    $("#getCtr").click(function(){     
		...
		str+="<td ><ul >";
				<?php if(is_array($texture_list)): foreach($texture_list as $key=>$v): ?>str+='<li style="float:left;"><div><input type="checkbox" name="info_price[newtexturelist]['+k+'][]" value="<?php echo ($v["id"]); ?>"><?php echo ($v["colorname"]); ?></div></li>';<?php endforeach; endif; ?>
		str+="</ul></td>";	
