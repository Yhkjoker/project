...
<?php }?>
        <div class="user_content">
            <div class="user_content_center">
                <h4>欢迎,以下是您的基本信息</h4>
                <div class="jiben">
                    <p>您的昵称：<?php if($vip_arr["nickname"] ): echo ($vip_arr["nickname"]); else: ?>未设置<?php endif; ?></p>
                    <p><span>宝宝的姓氏：<?php if($vip_arr["bbname"] ): echo ($vip_arr["bbname"]); else: ?>未设置<?php endif; ?></span><span>宝宝的年龄：<?php if($vip_arr["age"] ): echo ($vip_arr["age"]); else: ?>未设置<?php endif; ?></span></p>
                    <p>宝宝的性别：<?php if($vip_arr["sex"] ): if($vip_arr["sex"] == 0): ?>男<?php else: ?>女<?php endif; else: ?>未设置<?php endif; ?></p>
                    <p>您的e-mail：<?php if($vip_arr["email"] ): echo ($vip_arr["email"]); else: ?>未设置<?php endif; ?></p>
                    <p>您的地址：<?php if($vip_arr["address"] ): echo ($vip_arr["address"]); else: ?>未设置<?php endif; ?></p>
                </div>
                <?php if($vip_arr ): ?><a href="<?php echo U('Member/member_change');?>" class="user_content_xgzl"><?php else: ?><a href="javascript:dltc();" class="user_content_xgzl"><?php endif; ?>修改资料</a>
            </div>
        </div>
