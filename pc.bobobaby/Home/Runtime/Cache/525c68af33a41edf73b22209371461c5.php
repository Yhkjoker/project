...
<?php } ?>
        ...
        <div class="Product_list_content shiyong">
            ...
            <div class="Product_list_right f_right">
                ...
                <div class="Product_list_right_content survey">
				<form action="<?php echo U('Try/doQuestionnaire',array('id'=>$try_arr[id]));?>" method="post" class="myForm">
                  <h2><?php echo ($try_arr["title"]); ?>试用问卷</h2>
                    <ul>
						<?php echo ($form_list); ?>
                    </ul>
       
                        <input class="surveyBtn01" type="submit" value="提交" />
                        <input class="surveyBtn02" type="reset" value="重填" />
                    </form>
                </div>
	        </div>
	    </div>
...
<script>
			  ...
			  $('.myForm').Validform({
				...
				callback:function(data){
					...
					if(data.status==1){
						...
						setTimeout(function(){window.location="__APP__/Try/index/cls/2";},5000);
			  ...
			  $('.TryUseBg').click(function(){
				  $(this).hide();
				  $('#Validform_msg').hide();
				  window.location="__APP__/Try/index/cls/2";
